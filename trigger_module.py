...
class MAIN_MODULE_CLASS(COMMON_MAIN_MODULE_CLASS):
    ...
    def main(self):
        ...
        socket_check = context.socket(zmq.SUB)
        socket_check.connect('tcp://localhost:' + str(Port.proxy.value))
        socket_check.setsockopt(zmq.SUBSCRIBE, Subscription.signal.value)

        starting = True

        while not self.shutdown_event.is_set():
            if starting:
                logger.debug('load module params from html files')
                ...
                for rule_id in sorted(rule_dic.keys()):
                    ...
                    kwargs_list.append(
                        {'target': rule_picker, 'kwargs': {'rule_id': rule_id, 'formula_list': formula_list}})
                logger.debug('parameters loaded')

                threads_proc = Process(target=fps, args=(kwargs_list,))
                logger.debug('start threads_proc')
                threads_proc.start()
                logger.debug('process started')

            if socket_backend.poll(0):
